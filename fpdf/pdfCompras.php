...
class PDF extends FPDF
{
   ...
   function Header()
   {
      ...
      $consulta_info = $conexion->query(" select * from compras "); //traemos datos de la empresa desde BD
      ...
      $this->Cell(100, 10, utf8_decode("COMPRAS "), 0, 1, 'C', 0);
      $this->Ln(7);
      ...
      $this->Cell(50, 10, utf8_decode('ID_PRODUCTO'), 1, 0, 'C', 1);
      $this->Cell(80, 10, utf8_decode('NOMBRE_PRODUCTO'), 1, 0, 'C', 1);
...
$consulta_reporte_asistencia = $conexion->query(" select ref_producto,n_pro_compra,precio_c,unidades_c,fecha_c from compras");

while ($datos_reporte = $consulta_reporte_asistencia->fetch_object()) {
   $i = $i + 1;
   /* TABLA */
   $pdf->Cell(50, 10, utf8_decode($i), 1, 0, 'C', 0);
   $pdf->Cell(80, 10, utf8_decode($datos_reporte->n_pro_compra), 1, 0, 'C', 0);
   $pdf->Cell(30, 10, utf8_decode($datos_reporte->precio_c), 1, 0, 'C', 0);
   $pdf->Cell(50, 10, utf8_decode($datos_reporte->unidades_c), 1, 0, 'C', 0);
   $pdf->Cell(70, 10, utf8_decode($datos_reporte->fecha_c), 1, 1, 'C', 0);
}
