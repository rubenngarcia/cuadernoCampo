...
class PDF extends FPDF
{
   ...
   function Header()
   {
      ...
      $consulta_info = $conexion->query(" select * from fitosanitarios "); //traemos datos de la empresa desde BD
      ...
      $this->Cell(100, 10, utf8_decode("FITOSANITARIOS"), 0, 1, 'C', 0);
      $this->Ln(7);
      ...
      $this->Cell(20, 10, utf8_decode('ID_FI'), 1, 0, 'C', 1);
      $this->Cell(40, 10, utf8_decode('REF_FITOSANITARIO'), 1, 0, 'C', 1);
...
$consulta_reporte_asistencia = $conexion->query(" select ref_fitosanitario,longitud,latitud,técnico,fecha from fitosanitarios");

while ($datos_reporte = $consulta_reporte_asistencia->fetch_object()) {
   $i = $i + 1;
   /* TABLA */
   $pdf->Cell(20, 10, utf8_decode($i), 1, 0, 'C', 0);
   $pdf->Cell(40, 10, utf8_decode($datos_reporte->ref_fitosanitario), 1, 0, 'C', 0);
